...
import { buildPackedNetwork } from './packedNetwork.ts';
import type { PackedNetworkOptions } from './packedNetwork.ts';
import type { Simulation } from './types';

export type CordNetworkModel = 'elastic' | 'framed' | 'packed';
export type CordNetworkRequest = { simulation: Simulation; model: CordNetworkModel; options: FramedNetworkOptions & ElasticNetworkOptions & PackedNetworkOptions };
...
self.onmessage = (event: MessageEvent<CordNetworkRequest>) => {
  ...
  try {
    ...
    let lastPost = 0;
    const stream = (progress: number, build: () => CordNetworkLayout) => {
      const now = Date.now();
      if (now - lastPost < 300) return;                      // intermediate layouts, throttled
      lastPost = now;
      post({ layout: build(), progress, done: false });
    };
    const layout = model === 'packed' ? buildPackedNetwork(simulation, options, stream) : buildElasticNetwork(simulation, options, stream);
