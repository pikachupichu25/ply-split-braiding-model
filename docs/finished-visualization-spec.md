...
The finished craft grows from top to bottom on screen. In each gap column, a later cell's top may not be above the previous cell's top. Cells are processed chronologically and an already placed cell is immutable.

Two different runs of cells are formed, and they must not be confused:

- **A cord's run.** Successive splittee cells of the same stable cord propose a complete common-boundary edge join, including an opposite-lean reversal through the same gap. Rule 3 may move the new cell down when that join would overlap an earlier same-lean cell in its column. This is the run the finished surface makes visible.
...
For every continuing splittee in an adjacent gap or reversing through the same gap, align the new cell's incoming top corner with the previous cell's outgoing top corner. Since all vertical sides have the same length, this aligns both edge endpoints. Use the actual corner coordinates, including experimental stagger offsets. The previous outgoing edge lies at its event's `fromLane`; the new incoming edge lies at its event's `toLane`.

Each continuing splittee supplies an exact placement proposal. Never average translations across a row. If that proposal would overlap an earlier same-lean cell in the new cell's column, section 7.3 moves only the new cell downward and the continuity proposal yields. Acting as a splitter clears the previous visible endpoint, so an invisible splitter portion is never bridged.

Placement is chronological. Once a cell has been placed, no later event may translate it or a group containing it. An opposite-lean same-gap reversal remains eligible for the R2 shared-edge proposal; R3 adds no constraint between that pair.

When a later splitter action contains a previously visible splittee cord, that cord proposes the current cell's placement. The proposal never translates the rest of the action or any earlier event. If Rule 3 must push the current cell down, the proposed edge join is recorded as conflicted rather than moving cells already on the surface.
...
Use Braid's cord identities, lane exchanges, and participation order to identify the emergence. This Rule 4 relationship is the lowest-priority placement fallback: use it only when Rules 1, 2, and 3 place no constraint on the current cell. The top-to-bottom column-order invariant is then checked against the proposed R4 position; it may move the new cell farther down, but never moves an earlier cell.
...
Base cells are placed and drawn in event order, progressing from the top of the craft toward the bottom. Placing a later cell never changes the coordinates of an earlier cell. Vertical placement within a column depends on split direction:
...
- **Same split direction:** a later cell must not overlap any earlier cell with the same lean in that column. Edge contact and extra vertical space are both valid.
...
For each cell, first calculate the placement proposed by Rule 2, or by Rule 1 when Rule 2 does not apply. Do not allow the new cell's top to rise above the previously placed cell in its column. Then compare it with the most recently placed cell of its own lean in that column. If their interiors would overlap, move only the new cell down until their edges meet. If they already meet or have a gap, retain that placement. Because every same-lean cell is processed this way, the check also protects pairs separated by intervening opposite-lean events.

Do not impose an edge-separation constraint on opposite-direction pairs. Beyond the top-to-bottom column order, retain the positions inherited from the other rules, whether that produces partial overlap or open space. Rule 4 is considered when the current cell has no Rule 1, Rule 2, or Rule 3 relationship, and its proposal is then validated against column order.
...
- Cells are placed in event order; placing a new cell never changes an earlier cell's coordinates.
...
- Base cells in a column with the same split direction never overlap; edge contact and extra space are both valid, including for role-transition cells and same-gap returns.
...
- same-column base cells may partially overlap or leave open space when their split directions differ, while cells with the same split direction never overlap and may have extra space;
...
Prefix rendering is invariant: laying out the first `N` events on their own must produce exactly the same coordinates as the first `N` cells of any longer rendering of that event sequence.

Column-spacing acceptance checks must cover left-leaning and right-leaning pairs with both edge contact and extra space, always with no interior overlap, plus opposite-leaning pairs with partial overlap and pairs with open space at both boundaries. Include pairs separated by intervening opposite leans, role-transition cells, same-gap returns, and identical cord colours. Verify the rules at the angle-control limits and under Front/Back reflection while keeping exactly one cell per split event.
